...
var startAudio = new Audio('https://gluttonc.squareorbits.com/start.wav'); // Linked externally for bookmarklets
var deathAudio = new Audio('https://gluttonc.squareorbits.com/death.wav');
...
function gameLogic() {
	...
	if (started == 1) {
      ...
      /* This game originally used blank braille characters as a way to render whitespace in the urlbar, but as of Firefox 78 and Chrome 85, these characters are encoded. Unlike Chrome, Firefox doesn't encode spaces in its urlbar - only consecutive spaces get encoded. Inserting zero width joiners (normally used to join two or more characters together in sequence to create a new emoji) between each space used to get around this, but now unfortunately these too are encoded. */
